...
VombatBaseFilter::VombatBaseFilter(VombatFilterDescription desc, ccPluginInterface * parent_plugin)
	: m_action(0)
	, m_desc(desc)
...
void VombatBaseFilter::initAction()
{
	if (m_action)
		return;
...
void VombatBaseFilter::throwError(int errCode)
{
...
void VombatBaseFilter::updateSelectedEntities(const ccHObject::Container& selectedEntities)
{
...
int VombatBaseFilter::performAction()
{
...
int VombatBaseFilter::checkSelected()
{
...
static VombatBaseFilter* s_filter = 0;
...
QString VombatBaseFilter::getFilterName() const
{
	return m_desc.m_filter_name;
}

QString VombatBaseFilter::getEntryName() const
{
	return m_desc.m_entry_name;
}

QString VombatBaseFilter::getStatusTip() const
{
	return m_desc.m_status_tip;
}

QIcon VombatBaseFilter::getIcon() const
{
	return m_desc.m_icon;
}

QAction* VombatBaseFilter::getAction()
{
	return m_action;
}

QString VombatBaseFilter::getErrorMessage(int errorCode)
{
	QString errorMsg;
	switch(errorCode)
	{
...
ccPointCloud* VombatBaseFilter::getSelectedEntityAsCCPointCloud() const
{
...
ccHObject *VombatBaseFilter::getSelectedEntityAsCCHObject() const
{
	//does we have any selected entity?
	if (m_selected.size() == 0)
		return NULL;
...
ccHObject::Container VombatBaseFilter::getSelectedThatHaveMetaData(const QString key, const QString val) const
{
...
void VombatBaseFilter::getAllEntitiesOfType(CC_CLASS_ENUM type, ccHObject::Container& entities)
{
...
void VombatBaseFilter::getAllEntitiesThatHaveMetaData(QString key, ccHObject::Container &entities)
{
...
void VombatBaseFilter::getSelectedEntitiesThatAreCCPointCloud(ccHObject::Container & entities)
{
...
void VombatBaseFilter::getSelectedEntitiesThatAre(CC_CLASS_ENUM  kind, ccHObject::Container & entities)
{
	ccHObject::Container selected = m_selected;
	for (size_t i = 0 ; i < selected.size(); ++i)
	{
...
int VombatBaseFilter::hasSelectedScalarField(std::string field_name)
{
...
int VombatBaseFilter::hasSelectedScalarField()
{
...
std::vector<std::string> VombatBaseFilter::getSelectedAvailableScalarFields()
{
...
int VombatBaseFilter::isFirstSelectedCcPointCloud()
{
...
int VombatBaseFilter::hasSelectedRGB()
{
	if (isFirstSelectedCcPointCloud() != 1)
		return -1;
	//get the cloud
