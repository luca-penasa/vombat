...
ccPointPickingDlg::ccPointPickingDlg(QWidget* parent)
...
ccPointPickingDlg::~ccPointPickingDlg()
{
	if (m_label)
		delete m_label;
	m_label = 0;
...
bool ccPointPickingDlg::linkWith(ccGLWindow* win)
{
...
void ccPointPickingDlg::stop(bool state)
{
	assert(m_label && m_rect2DLabel);
...
void ccPointPickingDlg::onClose()
{
	stop(false);
}

void ccPointPickingDlg::activatePointPropertiesDisplay()
{
	if (m_associatedWin)
		m_associatedWin->setInteractionMode(ccGLWindow::TRANSFORM_CAMERA);
...
void ccPointPickingDlg::activateDistanceDisplay()
{
...
void ccPointPickingDlg::activateAngleDisplay()
{
...
void ccPointPickingDlg::activate2DZonePicking()
{
...
void ccPointPickingDlg::initializeState()
{
	assert(m_label && m_rect2DLabel);
	m_label->clear();
	m_rect2DLabel->setVisible(false);	//=invalid
...
void ccPointPickingDlg::exportCurrentLabel()
{
...
void ccPointPickingDlg::processPickedPoint(ccPointCloud* cloud, unsigned pointIndex, int x, int y)
{
	assert(cloud);
	assert(m_label);
...
void ccPointPickingDlg::processClickedPoint(int x, int y)
{
	if (m_pickingMode != RECT_ZONE)
		return;
...
void ccPointPickingDlg::update2DZone(int x, int y, Qt::MouseButtons buttons)
{
	if (m_pickingMode != RECT_ZONE)
		return;
...
void ccPointPickingDlg::close2DZone()
{
	if (m_pickingMode != RECT_ZONE)
		return;
