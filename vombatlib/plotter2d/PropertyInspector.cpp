...
void PropertyBrowser::setObject(QObject *object)
{
	...
	LOG(INFO) << "set object to: " << object;
	if (m_object == object)
		return;

	if (m_object || object == NULL) { // there was an object before.
									  //clean please
//		saveExpandedState();
...
void PropertyBrowser::slotValueChanged(QtProperty *property, const QVariant &value)
{
